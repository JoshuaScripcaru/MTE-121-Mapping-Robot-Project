...
void travelDistance(int distance, int motorPower, int & prevMotorEncoder) //assumes the robot travels forward
{
	...
	float rotateBy = distance/(DEGREES_TO_RADIANS*WHEEL_RADIUS); //*(180/(PI*2.75)); //finds how to rotate the wheels
...
void getMinAngle(int & minAngle, int & minDistance)
{
	...
	while(nMotorEncoder[ULTRASONIC_MOTOR] > -180*GEAR_RATIO) //finds angle relative to robot of the closest point to the wall
	{
		...
		motor[ULTRASONIC_MOTOR] = -ULTRASONIC_MOTOR_SPEED_SCAN/(ATAN_MAX_VALUE)*atan(curDistance*SCALE);
	}
...
task main()
{
	...
	if(count%1 == 0) //creates and updates file of map
	{
	bool fileOpen = putBoundaryIntoFile();
	if(!fileOpen)
	{
	displayString(5, "File did not open");
	}
	else
	{
	displayString(5, "              ");
